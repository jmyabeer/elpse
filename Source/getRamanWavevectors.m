function [srsBundle,epwBundle] = getRamanWavevectors(traj,omega_ps, ...
                                                          rayGd, ...
                                                          itmax,landauC)    
...
%      at some point I might add omea to the rayBundle
    ...
    phi = 1;
    for iter = 1:itmax
        % k is plasma wave wavevector
        k =  k0 + kvac*sqrt(1-2*wpeTow0.*phi);          % um^-1
        % thermal correction to k
        k2lam2 = (k.^2).*lamDebum2;
        phi = 1 + 3/2*k2lam2.*(1-sqrt(neTonc));
    end
    
    kLangmuir = [k k].*k0Hat;    % k is magnitude for each point
    ks = (k0-k);                 % magnitude of raman light k vector
    kSRS = [ks ks].*k0Hat;

    % can't have Raman just anywhere ...
    goodidxs = find((imag(k)==0) & (abs(k2lam2) <= landauC^2));
