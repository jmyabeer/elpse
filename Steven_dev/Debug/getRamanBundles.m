function [srsBundle,epwBundle] = getRamanBundles(traj,omega_ps,rayGd,angle, ...
                                                          itmax,landauC)    
% GETRAMANBUNDLES - documentation to do
%
...
%      at some point I might add omea to the rayBundle
    ...
    % Calculate entrance angle of the ray in degrees
    IncAng = atan2d(traj(1,5),traj(1,4)); 
    
    % Takes the sign of kr of the first point
    krSign = sign(traj(1,5));
    
    % can't have Raman just anywhere ...
    % Finds points where k is real, Landau damping isn't an issue, and the
    % ray hasn't deflected of the target
    goodidxs = find((imag(kSRSMag)==0) & (abs(k2lam2) <= landauC^2) & ...
       (atan2d(traj(:,5),traj(:,4)) < (IncAng+10)) ...
       & (atan2d(traj(:,5),traj(:,4)) > (IncAng-10)) & ...
       (sign(traj(:,5)) == krSign));
